...
WITH [tmp_dates] AS (
  ...
  SELECT -- TOP 100
      ROW_NUMBER() OVER (ORDER BY [DATE_DOWN] desc) AS DATE_NUM
      , [DATE_DOWN]
    FROM (
      SELECT DISTINCT
          [DATE_DOWN]
...
, [tmp_subtables] AS (
  ...
  SELECT TOP (1000000)
      ROW_NUMBER() OVER (PARTITION BY t_dates.[DATE_DOWN] ORDER BY tt.[Id]) as ROW_NUM
      , t_dates.[DATE_NUM]
      , t_dates.[DATE_DOWN]
      , tt.[uniq_num]
      , tt.[fio_client]
      ...
      , tt.[ddu_date_sign]
      , tt.[status]
    FROM [tmp_dates] AS t_dates
      INNER JOIN [dbo].[xls_accounts_receivable] AS tt
      ON t_dates.DATE_DOWN = tt.DATE_DOWN
    ORDER BY t_dates.[DATE_DOWN] desc, [ROW_NUM]
)
, [tmp_compares] AS (
